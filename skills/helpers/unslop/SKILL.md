...
1. **Em dash overuse.** Avoid em dashes entirely. Use periods or commas only (no parentheses, no hyphen-as-dash substitutes). Em dashes are an AI tell, and reaching for parentheses instead just trades one tell for another. If a thought needs separation, end the sentence or use a comma. En dashes in numeric ranges (1-10, 3-5) are fine.
